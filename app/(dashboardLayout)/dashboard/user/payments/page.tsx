        ...
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-[#2a2d38] text-white">
              <TableRow>
                {['Transaction ID', 'Amount', 'Method', 'Status', 'Date'].map((head) => (
                  <TableHead key={head} className="text-white border border-slate-700">
                    {head}
                  </TableHead>
                ))}
              </TableRow>
            ...
            <TableBody>
              {paymentsData?.data?.length === 0 && (
                <TableRow>
                  <TableCell colSpan={6} className="text-center p-4 text-foreground border border-slate-700">
                    No payments found.
                  </TableCell>
                </TableRow>
              )}
              {paymentsData?.data?.map((payment: any) => (
                ...
                >
                  ...
                  <TableCell className="border border-slate-700">
                    <Badge
                      className={`text-white capitalize ${payment.status === 'success'
                          ? 'bg-emerald-600'
